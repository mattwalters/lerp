...
func (k keymap) FullHelp() [][]key.Binding {
	return [][]key.Binding{
		...
		{k.Detail, k.Close, k.Promote, k.Eject, k.ForceStart, k.Sort, k.Project,
...
//
// hasLog, hasURL, canPromote and canEject say which of these keys the row
// under the cursor actually answers to: r is inert on a ticket that has never
// run, o on a run whose ticket the pass no longer lists, p where there is no
// status to promote into or no room for the picker the key opens, and e on
// anything but a live run under a runner that can resume. An advertised key
...
func (k keymap) panelHelp(p panel, hasLog, hasURL, canPromote, canEject bool) []key.Binding {
	var b []key.Binding
	switch p {
	case panelAttention:
