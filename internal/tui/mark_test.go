...
func TestTheWordmarkStaysOffScreenBehindProjectFilter(t *testing.T) {
	...
	// Slice to reach the project list, scope to "Core", and slice back to all:
	m = update(t, m, keyMsg("]"))
	m = update(t, m, keyMsg("P"))
	...
	if m.filterField != filterFieldProject || m.filterValue != "Core" || len(m.shown) != 0 {
		t.Fatalf("setup: slicing changed the scope: field=%v val=%q, %d rows", m.filterField, m.filterValue, len(m.shown))
	}
	if hasDimMark(m.View()) {
